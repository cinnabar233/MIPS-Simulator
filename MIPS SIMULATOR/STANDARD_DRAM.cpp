...
void evaluate()
{
    ...
    while(i <= n)
    {
       ...
       // print_reg_values(i) ; //print register values after executing ith iteration
        ...
        else if(kind == "beq") {if(Register[Memory(i).args[0]] == Register[Memory(i).args[1]]) {
            cout << clock_cycles <<":";print_instruction(Memory(i));cout<<"\n\n";i = lab_addr[Memory(i).label]; continue;}}
        
        else if(kind == "bne") {if(Register[Memory(i).args[0]] != Register[Memory(i).args[1]]) {
            cout << clock_cycles <<":";print_instruction(Memory(i));cout<<"\n\n";i = lab_addr[Memory(i).label]; continue;}}
        ...
        if(kind == "lw" or kind == "sw")
        {
            ...
            if(parts.size()== 3 && x!=-1){
                ...
                if(y.second == 1  && z!=-1 && parts[2].size()==0){
                    ...
                    store_instructions(n,{1,kind,"",{x,y.first/4,z}});
                    return true;
                }
                else return false;
            }
...
int main(int argc , char** argv)
{
    ...
    // output file
    freopen("output_SDRAM.txt","w", stdout);
